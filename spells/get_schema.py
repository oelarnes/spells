...
COLUMN_TYPES = (
	...
	(re.compile(r"^((user)|(oppo))_turn_\d+_((user)|(oppo))_instants_sorceries_cast$"), pl.String),
	(re.compile(r"^((user)|(oppo))_turn_\d+_((user)|(oppo))_abilities$"), pl.String),
	(re.compile(r"^((user)|(oppo))_turn_\d+_((user)|(oppo))_cards_learned$"), pl.String),
	...
	(re.compile(r"^((user)|(oppo))_turn_\d+_player_combat_damage_dealt$"), pl.String),  # DEPRECATED
	(re.compile(r"^((user)|(oppo))_turn_\d+_((user)|(oppo))_combat_damage_taken$"), pl.String),
	(re.compile(r"^((user)|(oppo))_turn_\d+_((user)|(oppo))_creatures_killed_combat$"), pl.String),
	(re.compile(r"^((user)|(oppo))_turn_\d+_((user)|(oppo))_creatures_killed_non_combat$"), pl.String),
	...
	(re.compile(r"^((user)|(oppo))_turn_\d+_eot_((user)|(oppo))_lands_in_play$"), pl.String),
	(re.compile(r"^((user)|(oppo))_turn_\d+_eot_((user)|(oppo))_creatures_in_play$"), pl.String),
	(re.compile(r"^((user)|(oppo))_turn_\d+_eot_((user)|(oppo))_non_creatures_in_play$"), pl.String),
	(re.compile(r"^((user)|(oppo))_turn_\d+_eot_((user)|(oppo))_life$"), pl.Float64),
	(re.compile(r"^((user)|(oppo))_turn_\d+_eot_((user)|(oppo))_poison_counters$"), pl.Float64),
...
def schema(filename: str, print_missing: bool = False) -> Dict[str, pl.datatypes.DataType] | None:
