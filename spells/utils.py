...
def convert_to_expr_list(input: str | pl.Expr | list[str | pl.Expr] | None):
    if input is None:
        return []
...
def wavg(
    ...
    assert len(name_list) == len(
        col_list
    ), f"{len(name_list)} names provided for {len(col_list)} columns"
    assert len(name_list) == len(set(name_list)), "Output names must be unique"
    assert (
        len(weight_list) == len(col_list) or len(weight_list) == 1
    ), f"{len(weight_list)} weights provided for {len(col_list)} columns"

    enum_wl = weight_list * int(len(col_list) / len(weight_list))
    wl_names = [w.meta.output_name() for w in weight_list]
    assert len(wl_names) == len(
        set(wl_names)
    ), "Weights must have unique names. Send one weight column or n uniquely named ones"
    ...
    ret_df = grouped.sum().select(
        ...
        + wl_names
        + [
            (pl.col(name) / pl.col(enum_wl[i].meta.output_name()))
