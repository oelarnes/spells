...
matching the data files. `_normalize_context_keys` takes a user-provided context — given
...
from spells.draft_data import _normalize_context_keys
from spells.enums import EventType


def test_none_returns_none_for_every_cell():
    assert _normalize_context_keys(None, [("TST", EventType.PREMIER)]) == {
...
def test_bare_set_context_broadcasts_to_every_cell():
    ...
    assert _normalize_context_keys(ctx, cells) == {cell: ctx for cell in cells}
...
def test_bare_card_context_broadcasts_to_every_cell():
    ...
    assert _normalize_context_keys(ctx, [cell]) == {cell: ctx}
...
def test_set_code_keyed_broadcasts_across_event_types():
    ...
    resolved = _normalize_context_keys(ctx, cells)
...
def test_tuple_keyed_accepts_plain_string_event_type():
    ...
    assert _normalize_context_keys(ctx, [("TST", EventType.PREMIER)]) == {
        ("TST", EventType.PREMIER): {"picks_per_pack": 14}
    }
    assert _normalize_context_keys(ctx, [("TST", "PremierDraft")]) == {
...
def test_tuple_keyed_missing_cell_returns_none():
    ...
    assert _normalize_context_keys(ctx, [("TST", EventType.TRADITIONAL)]) == {
...
def test_dataframe_filters_by_expansion():
    ...
    resolved = _normalize_context_keys(df, cells)
